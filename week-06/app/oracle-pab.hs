...
-- Different to Emulator Trace because this it's possible to make IO
main :: IO ()
main = void $ Simulator.runSimulationWith handlers $ do
    ...
    -- To interact with the oracle from UI write handler into a file oracle.cid
    cidOracle <- Simulator.activateContract (Wallet 1) $ Oracle cs
    liftIO $ writeFile "oracle.cid" $ show $ unContractInstanceId cidOracle
    -- Wait until NFT is minted and oracle is created
    oracle <- waitForLast cidOracle

    -- Enable swap contracto for each wallet (except wallet 1)
    forM_ wallets $ \w ->
        when (w /= Wallet 1) $ do
