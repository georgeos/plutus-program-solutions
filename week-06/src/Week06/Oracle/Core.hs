...
data Oracle = Oracle
    ...
    , oFee      :: !Integer
    , oAsset    :: !AssetClass  -- Represents an Asset Class to exchange ADA, e.g. USDT
...
updateOracle oracle x = do
    ...
    case m of
        ...
        Just (oref, o,  _) -> do
            ...
            -- Because only NFT is attached into the Output, then there is an inbalance, so automatically all existing fees are paid to the owner
...
findOracle oracle = do
    -- Finds all UTXO sitting at the oracleAddress
    -- Map.filter : Data.Map
    ...
    return $ case Map.toList utxos of
        ...
        _           -> Nothing
  where
    -- Looks for a UTXO which contains exactly one NFT
    f :: TxOutTx -> Bool
