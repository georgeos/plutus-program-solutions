...
instance ContractModel TSModel where
    ...
    nextState (BuyTokens v w n) = do
        when (n > 0) $ do
            ...
            case m of
                Just t
                    | t ^. tssToken >= n && Prelude.not (t ^. tssFinished) -> do
                        let p = t ^. tssPrice
                            l = p * n
    ...
    nextState (Withdraw v w n l) = do
        when (v == w) $ do
            ...
            case m of
                Just t
                    | t ^. tssToken >= n && t ^. tssLovelace >= l && Prelude.not (t ^. tssFinished) -> do
            ...
            case m of
                Just t
                    | Prelude.not (t ^. tssFinished) -> do
                        deposit w $ lovelaceValueOf l <> assetClassValue (tokens Map.! w) n
                        (tsModel . ix v . tssLovelace) $~ (+ (- l))
                        (tsModel . ix v . tssToken)    $~ (+ (- l))
                        (tsModel . ix v . tssFinished) $~ finished
                        where
                            l = t ^. tssLovelace
                            n = t ^. tssToken
                _ ->  return ()
    ...
    perform h _ cmd = case cmd of
        ...
        (Close v w)        -> callEndpoint @"close"      (h $ UseKey v w) Nothing                                              >> delay 1
